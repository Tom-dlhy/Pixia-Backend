...
class EvaluationOuput(BaseModel):
    id: Optional[str] = Field(None, description="Identifiant unique de l'évaluation")
    exercice : ExerciseOutput = Field(..., description="Exercices générés pour l'évaluation")
...
class Chapter(BaseModel):
    ...
    exercice : ExerciseOutput = Field(None, description="Exercices associés au chapitre")
    evaluation : EvaluationOuput = Field(None, description="Évaluation associée au chapitre")
...
class DeepCourseSynthesis(BaseModel):
    ...
    plan_chapters : List[ChapterSynthesis] = Field(..., min_length=1, max_length=16,description="Liste des plans de chapitres du deepcourse")
