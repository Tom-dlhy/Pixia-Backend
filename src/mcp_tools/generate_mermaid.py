...
def generate_schema_mermaid(mermaid_code: str):
    ...
    out_path = os.path.join(folder_path, f"mermaid_{digest}.png")
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(
            f"curl a échoué (exit {proc.returncode}) : {err or 'aucun message'}"
        )
    try:
...
def generate_mermaid_schema_description(course_part: Part):
    ...
    try:
        ...
        return PartSchema(
            id_schema=uuid_schema, id_part=course_part.id_part, img_base64=img_b64
        )
    except Exception as e:
