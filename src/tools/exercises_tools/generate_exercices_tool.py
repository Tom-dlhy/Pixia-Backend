...
async def generate_exercises(
    ...
    with Timer(f"Exercices: {synthesis.title}"):
        ...
        timeout_seconds = (
            60  # Timeout de 60 secondes par tentative (Gemini peut être lent)
        )
        plan_json = None
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(
                    f"⏱️ Timeout après {timeout_seconds}s (tentative {attempt + 1}/{max_retries})"
                )
        ...
        for idx, r in enumerate(results):
            ...
            if isinstance(r, dict):
                if not r or "type" not in r:
                    logger.warning(
                        f"⚠️ Exercice {idx + 1}/{len(results)} est un dict vide ou sans 'type', ignoré"
                    )
                    continue
        ...
        if not generated_exercises:
            logger.error(
                f"❌ Aucun exercice valide généré sur {len(results)} tentatives"
            )
            return GenerativeToolOutput(
        ...
        logger.info(
            f"✅ {len(generated_exercises)}/{len(results)} exercices valides générés"
        )
