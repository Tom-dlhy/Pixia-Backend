...
async def download_course(
    ...
    try:
        ...
        if not test_course:
            ...
            raise HTTPException(
                status_code=404, 
                detail="Cours non trouvé pour ce session_id"
            )
        ...
        objet_course = CourseOutput(**course_data)

        logger.info(f"� Génération PDF pour: {objet_course.title}")
        ...
        raise HTTPException(
            status_code=500, 
            detail="Erreur lors du parsing du contenu du cours"
        )
